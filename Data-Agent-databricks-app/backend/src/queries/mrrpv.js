...
export async function getMRRpV(params = {}) {
  ...
  const productKeys = includeProduct == null ? [] : Array.isArray(includeProduct) ? includeProduct.map((p) => String(p).toLowerCase()) : [String(includeProduct).toLowerCase()];
  const productCountCols = productKeys
    .map((key) => PRODUCT_COUNT_COLUMNS[dataSource]?.[key])
    .filter(Boolean);
  if (productKeys.length > 0 && productCountCols.length !== productKeys.length) {
    const allowed = Object.keys(PRODUCT_COUNT_COLUMNS[dataSource] || {}).join(', ');
    throw new Error(`One or more products are not supported for ${dataSource} (include filter). Allowed: ${allowed || 'none'}`);
  }
  ...
  if (productCountCols.length > 0) {
    where.push(`(${productCountCols.map((c) => `(${c} > 0)`).join(' AND ')})`);
  }
...
export async function getBridgeMRRpV(params = {}) {
  const { timeWindow, filters = {}, includeProduct } = params;
  ...
  const bridgeProductKeys = includeProduct == null ? [] : Array.isArray(includeProduct) ? includeProduct.map((p) => String(p).toLowerCase()) : [String(includeProduct).toLowerCase()];
  const bridgeProductCountCols = bridgeProductKeys
    .map((key) => PRODUCT_COUNT_COLUMNS.first_purchase?.[key])
    .filter(Boolean);
  if (bridgeProductKeys.length > 0 && bridgeProductCountCols.length !== bridgeProductKeys.length) {
    const allowed = Object.keys(PRODUCT_COUNT_COLUMNS.first_purchase || {}).join(', ');
    throw new Error(`One or more products are not supported for bridge (include filter). Allowed: ${allowed || 'none'}`);
  }
  const regionsList = Array.isArray(filters.regions) ? filters.regions.filter((g) => g != null && String(g).length <= 100) : [];
  const excludeRegionsList = Array.isArray(filters.excludeRegions) ? filters.excludeRegions.filter((g) => g != null && String(g).length <= 100) : [];
  const segmentsList = Array.isArray(filters.segments) ? filters.segments.filter((s) => s != null && String(s).length <= 100) : [];
  ...
  if (bridgeProductCountCols.length > 0) {
    where.push(`(${bridgeProductCountCols.map((c) => `(${c} > 0)`).join(' AND ')})`);
  }
  if (regionsList.length > 0) {
    const escaped = regionsList.map((g) => `'${String(g).replace(/'/g, "''")}'`);
    where.push(`(geo IN (${escaped.join(', ')}))`);
  ...
  // All Other Contribution = Route Planning (rp_acv) + Platform Pro + MEM only; not the table's other_acv bucket
  ...
  const { columns: rawColumns, rows: rawRows } = await runSql(sql);
  const rawColNames = rawColumns?.length > 0 ? rawColumns.map((col) => col.name) : ['close_quarter', 'vehicle_count', 'acv', 'fleet_mrrpv', 'vg_core_acv', 'vg_count', 'cm_core_acv', 'cm_count', 'vgcm_core_acv', 'vgcm_addon_acv', 'st_acv', 'flapps_acv', 'cc_acv', 'rp_acv', 'subsidy_acv'];
  const rawColCount = rawColNames.length;
  let rows = rawRows;
  ...
  const data = (rows || []).map((row) => {
    ...
    totals.rp_acv += rp_acv != null ? Number(rp_acv) : 0;
    ...
    // All Other Contribution = Route Planning (rp_acv) + Platform Pro + MEM only; add platform_pro_acv, mem_acv when columns exist
    const other_contribution = monthlyPerVehicle(rp_acv) != null ? Math.round(monthlyPerVehicle(rp_acv) * 100) / 100 : null;
    return {
      close_quarter,
  ...
  const gtOther = vcTotal > 0 ? Math.round((totals.rp_acv / 12 / vcTotal) * 100) / 100 : null;
  const grandTotals = {
...
/**
 * First-purchase product key → ACV and count column names (for ASP = acv/count/12).
 * Only products with both columns in first_purchase table are supported.
 */
const ASP_PRODUCT_COLUMNS = {
  cm: { acv: 'cm_core_acv', count: 'cm_count' },
  vg: { acv: 'vg_core_acv', count: 'vg_count' },
  aim4: { acv: 'aim4_acv', count: 'aim4_count' },
  st: { acv: 'st_acv', count: 'st_count' },
  cam: { acv: 'cam_acv', count: 'cam_count' },
  flapps: { acv: 'flapps_acv', count: null },
  cc: { acv: 'cc_acv', count: null },
  cw: { acv: 'cw_acv', count: 'cw_count' },
  ct: { acv: 'ct_acv', count: 'ct_count' },
  cnav: { acv: 'cnav_acv', count: 'cnav_count' },
  rp: { acv: 'rp_acv', count: 'rp_count' },
};

/** Product key → display label for ASP view rows. */
const ASP_PRODUCT_LABELS = {
  cm: 'Safety',
  vg: 'Telematics',
  aim4: 'AIM4',
  st: 'Smart Trailers',
...
/**
 * ASP Investigation view: time series (quarters as columns), rows = product ASP and ACV per product.
 * Only supports first_purchase. Returns { columns, data, viewType: 'asp' }.
 * @param {{ timeWindow: string, products?: string[], filters?: object }} params
 *   products: e.g. ['cm','vg','aim4']. Default ['cm','vg','aim4'].
 */
export async function getASPInvestigation(params = {}) {
  const { timeWindow, products: rawProducts = ['cm', 'vg', 'aim4'], filters = {} } = params;
  const config = getSourceConfig('first_purchase');
  const periods = resolveTimeWindows(timeWindow);
  if (!periods || periods.length === 0) {
    throw new Error('ASP view requires timeWindow (e.g. FY26 Q1,FY26 Q2,FY26 Q3,FY26 Q4)');
  }
  const products = (Array.isArray(rawProducts) ? rawProducts : [rawProducts])
    .map((p) => String(p).toLowerCase())
    .filter((p) => ASP_PRODUCT_COLUMNS[p]);
  if (products.length === 0) {
    throw new Error('ASP view requires at least one product. Supported: ' + Object.keys(ASP_PRODUCT_COLUMNS).join(', '));
  }

  const regionsList = Array.isArray(filters.regions) ? filters.regions.filter((g) => g != null && String(g).length <= 100) : [];
  const excludeRegionsList = (Array.isArray(filters.excludeRegions) ? filters.excludeRegions : Array.isArray(filters.exclude_regions) ? filters.exclude_regions : []).filter((g) => g != null && String(g).length <= 100);
  const segmentsList = Array.isArray(filters.segments) ? filters.segments.filter((s) => s != null && String(s).length <= 100) : [];
  const excludeSegmentsList = (Array.isArray(filters.excludeSegments) ? filters.excludeSegments : Array.isArray(filters.exclude_segments) ? filters.exclude_segments : []).filter((s) => s != null && String(s).length <= 100);
  const industriesList = Array.isArray(filters.industries) ? filters.industries.filter((i) => i != null && String(i).length <= 100) : [];
  const excludeIndustriesList = (Array.isArray(filters.excludeIndustries) ? filters.excludeIndustries : Array.isArray(filters.exclude_industries) ? filters.exclude_industries : []).filter((i) => i != null && String(i).length <= 100);
  ...
  const selectParts = [timeCol];
  for (const key of products) {
    const spec = ASP_PRODUCT_COLUMNS[key];
    if (spec.acv) selectParts.push(`SUM(${spec.acv}) AS ${key}_acv`);
    if (spec.count) selectParts.push(`SUM(${spec.count}) AS ${key}_count`);
  }
  const sql = `SELECT ${selectParts.join(', ')} FROM ${fullTable}${whereClause} GROUP BY ${timeCol} ORDER BY ${timeCol} LIMIT 100`;
  const { columns: rawColumns, rows: rawRows } = await runSql(sql);
  const rawColNames = rawColumns?.length > 0 ? rawColumns.map((c) => c.name) : [timeCol, ...products.flatMap((k) => {
    const s = ASP_PRODUCT_COLUMNS[k];
    return [s?.acv && `${k}_acv`, s?.count && `${k}_count`].filter(Boolean);
  })];
  const rawColCount = rawColNames.length;
  let rows = rawRows;
  if (rawRows?.length > 0 && Array.isArray(rawRows[0])) {
    rows = rawRows;
  } else if (rawRows?.length > 0 && !Array.isArray(rawRows[0]) && rawColCount > 0 && rawRows.length % rawColCount === 0) {
    rows = [];
    for (let i = 0; i < rawRows.length; i += rawColCount) {
      rows.push(rawRows.slice(i, i + rawColCount));
    }
  ...
  for (const row of rows) {
    const obj = {};
    rawColNames.forEach((name, idx) => {
      obj[name] = Array.isArray(row) ? row[idx] : row[name];
    });
    const q = obj[timeCol];
    if (q) byQuarter[q] = obj;
  }

  const colNames = ['row_label', ...periods];
  const data = [];
  for (const key of products) {
    const label = ASP_PRODUCT_LABELS[key] || key;
    const spec = ASP_PRODUCT_COLUMNS[key];
    const hasCount = spec && spec.count;
    if (hasCount) {
      const aspRow = { row_label: `${label} ASP` };
      const acvRow = { row_label: `${label} ACV` };
      for (const q of periods) {
        const r = byQuarter[q] || {};
        const acv = r[`${key}_acv`] != null ? Number(r[`${key}_acv`]) : null;
        const count = r[`${key}_count`] != null ? Number(r[`${key}_count`]) : null;
        const asp = count > 0 && acv != null ? Math.round((acv / count / 12) * 100) / 100 : null;
        aspRow[q] = asp;
        acvRow[q] = acv;
      }
      data.push(aspRow, acvRow);
    } else if (spec?.acv) {
      const acvRow = { row_label: `${label} ACV` };
      for (const q of periods) {
        const r = byQuarter[q] || {};
        acvRow[q] = r[`${key}_acv`] != null ? Number(r[`${key}_acv`]) : null;
      }
      data.push(acvRow);
    }
  }
  const columns = colNames.map((name) => ({ name }));
  return { columns, rows: data, data, viewType: 'asp' };
}
