...
router.post('/mrrpv', async (req, res) => {
  ...
  try {
    const body = req.body || {};
    const { dataSource, timeWindow, groupBy, filters, includeProduct, includeAccountCount, includeAvgDealSize, viewType, presetId, products } = body;
    ...
    const result = useBridge
      ? await getBridgeMRRpV({ timeWindow, filters: filters || {}, includeProduct })
      : useASP
        ? await getASPInvestigation({ timeWindow, products: products || ['cm', 'vg', 'aim4'], filters: filters || {} })
    ...
    logUsage({
      ...
      toolCalls: [{ name: useBridge ? 'get_bridge_mrrpv' : useASP ? 'get_mrrpv' : 'get_mrrpv', args: useBridge ? { time_window: timeWindow, include_product: includeProduct, ...filters } : useASP ? { time_window: timeWindow, view_type: 'asp', products: products || ['cm', 'vg', 'aim4'], ...filters } : { time_window: timeWindow, group_by: groupBy, include_product: includeProduct, include_account_count: includeAccountCount, include_avg_deal_size: includeAvgDealSize, ...filters } }],
