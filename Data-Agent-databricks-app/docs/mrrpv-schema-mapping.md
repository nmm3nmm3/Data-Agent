...
| **ENT - SEL** | Select |
| **ENT - STR** | Strategic |
| **US - SLED-MM** | Public Sector, Government, Public Sector MM, Public Sector Mid Market, Government MM, Government Mid Market |
| **US - SLED-ENT - SEL** | Public Sector, Government, Public Sector SEL, Public Sector Select, Government SEL, Government Select |
...
**Bridge "All Other Contribution":** In the MRRpV Bridge view, "All Other Contribution" is **not** the table column `other_acv`. It is defined as **Route Planning + Platform Pro + MEM** only. Implemented as: `rp_acv` (Route Planning); add `platform_pro_acv` and `mem_acv` (or equivalent column names) when those columns exist in the first_purchase table. Contribution = (annual ACV / 12) / vehicle_count (monthly $ per vehicle).

**ASP Investigation view:** A preset table (id: `asp-investigation`) shows **ASP** (Average Selling Price = product ACV / product count / 12, monthly $ per license) and **ACV** by product with **time series as columns** (e.g. FY26 Q1, Q2, Q3, Q4). Rows = one row per product for "Product ASP" and one for "Product ACV" (e.g. Safety ASP, Safety ACV, Telematics ASP, Telematics ACV, AIM4 ASP, AIM4 ACV). Default products: Safety (cm), Telematics (vg), AIM4 (aim4). The view supports: **add/remove products** (e.g. "add Maintenance" → include `cam`; "add Smart Trailers" → include `st`; **Maintenance = cam**, not st), **change timeframe** (time_window), and **limit by segment, geo, or industry** (regions, exclude_regions, segments, exclude_segments, industries, exclude_industries). Resolve plain-English to table values per this doc. Product keys for ASP: cm, vg, aim4, st, cam (Maintenance), flapps, cc, cw, ct, cnav, rp (only products with both ACV and count columns get an ASP row).
...
| Date | Change |
|------|--------|
| 2026-02-27 | Bridge "All Other Contribution" defined as Route Planning + Platform Pro + MEM only; implemented using rp_acv (add platform_pro_acv, mem_acv when columns exist). Documented in Product/feature columns section. |
| 2026-02-27 | Added segment table values US - SLED-MM and US - SLED-ENT - SEL with plain-English mappings (Public Sector/Government MM and SEL/Select). |
